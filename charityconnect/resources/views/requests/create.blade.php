                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select id="request_type" name="request_type" class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 block w-full mt-1" required>
                                    <option value="drop-off" {{ old('request_type') == 'drop-off' ? 'selected' : '' }}>Drop-off (Donor brings items to you)</option>
                                    <option value="pickup" {{ old('request_type') == 'pickup' ? 'selected' : '' }}>Pickup (You collect items from donor)</option>
                                </select>
                            ...
                            <div>
                                ...
                                <x-text-input id="drop_off_address" class="block mt-1 w-full" type="text" name="drop_off_address" :value="old('drop_off_address')" required />
                            ...
                            <div>
                                ...
                                <x-text-input id="available_times" class="block mt-1 w-full" type="date" name="available_times" :value="old('available_times')" required />
                                <x-input-error :messages="$errors->get('available_times')" class="mt-2" />
                            </div>
                        </div>

                        <div class="flex items-center justify-end mt-6">
                            <a href="{{ route('requests.index') }}" class="text-gray-500 hover:text-gray-700 mr-4">
                                {{ __('Cancel') }}
                            </a>
                            <x-primary-button>
                                {{ __('Create Request') }}
